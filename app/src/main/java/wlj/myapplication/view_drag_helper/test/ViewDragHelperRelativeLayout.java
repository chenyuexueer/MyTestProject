 ...
 * 时间： 2018/9/14.
 * 说明：自定义ViewGroup 侧滑删除
 * ================================================
 */
public class ViewDragHelperRelativeLayout extends RelativeLayout {
    ...
    public static interface OnSwipeChangeListener {
        void onDraging(ViewDragHelperRelativeLayout mViewDragHelperRelativeLayout);

        void onOpen(ViewDragHelperRelativeLayout mViewDragHelperRelativeLayout);

        void onClose(ViewDragHelperRelativeLayout mViewDragHelperRelativeLayout);

        void onStartOpen(ViewDragHelperRelativeLayout mViewDragHelperRelativeLayout);

        void onStartClose(ViewDragHelperRelativeLayout mViewDragHelperRelativeLayout);

    }

    //重写三个构造方法
    public ViewDragHelperRelativeLayout(Context context) {
        this(context, null);
    }

    public ViewDragHelperRelativeLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public ViewDragHelperRelativeLayout(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private ViewDragHelper.Callback callback = new ViewDragHelper.Callback() {
        ...
        public int clampViewPositionHorizontal(View child, int left, int dx) {
            Log.e("自定义RelativeLayout==", "left:" +left + ", dx:" + dx) ;
            if (child == frontView) {
                if (left > 0) {
                    return 0;
                } else if (left < -range) {
                ...
                if (left > width) {
                    Log.e("自定义RelativeLayout==", "left"+left +" width:" + width );
                    return width;
                } else if (left < width - range) {
                    Log.e("自定义RelativeLayout==", "left"+left +" width - range:" + (width - range));
                    return width - range;
                }else {
                    Log.e("自定义RelativeLayout==", "else情况" + "left"+left +" width:" + width );
                }
            }
            return left;
        }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        Log.e("自定义RelativeLayout==","height :" + height + "  width :" + height + "  range :" + height);
