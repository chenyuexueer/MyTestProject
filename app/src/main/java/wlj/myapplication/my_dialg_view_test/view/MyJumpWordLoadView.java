...
public class MyJumpWordLoadView extends android.support.v7.widget.AppCompatTextView {
    //文本绘制笔
    private Paint textPaint;
    //影子绘制笔
    private Paint shadowPaint;
    //参数项
    private MyJumpWordLoadViewOption option;
    //字体大小
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if(option!=null&&str!=null&&str.length()>0){
            if(option.showType == MyJumpWordLoadViewOption.SHOW_TYPE_ALL){
            ...
            if(Y>maxY[index]){
                Y = maxY[index];
                if(option.cycleType== MyJumpWordLoadViewOption.CYCLE_TYPE_FIFO){
                    index++;
                    index %= str.length();
                }else{
    ...
    private int getTextX(int i){
        switch (option.showType){
            case MyJumpWordLoadViewOption.SHOW_TYPE_ALL:
                int w = (getWidth()-textSize)/str.length();
                return w*i+textSize;
            case MyJumpWordLoadViewOption.SHOW_TYPE_ONCE:
                return getWidth()/2;
        }
        return 0;
    }
    ...
    public MyJumpWordLoadView(Context context) {
        this(context,null);
    }

    public MyJumpWordLoadView(Context context, AttributeSet attrs) {
        this(context, attrs,0);
    }

    public MyJumpWordLoadView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void setOption(MyJumpWordLoadViewOption option) {
        this.option = option;
        str = option.values;
        isInit = false;
        requestLayout();
