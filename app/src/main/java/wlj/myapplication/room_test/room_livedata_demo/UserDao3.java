...
public interface UserDao3 {
    ...
    LiveData<List<User3>> getUser3All();//查询方法返回,这里livedata观察，是观察数据变化而不是观察查询方法
    ...
    List<User3> getUser3Byids(int[] uids);//查询方法返回
    ...
    User3 getUser3ByFirst_Last_Name(String firstName, String lastName);
     ...
     */
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    List<Long> insertUser3s1(User3... users);
     ...
     */
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    List<Long> insertUser3s2(List<User3> users);
    //end------------------------insert插入------------------------
     ...
     */
    @Update()
    int updateUser3s2(List<User3> user);
    //end------------------------update更新------------------------
     ...
     */
    @Delete
    int deleteUser3s1(List<User3> users);
